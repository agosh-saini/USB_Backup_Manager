...
class DBUtils:
    ...
    def get_backup_key(self, account_id: str) -> Optional[str]:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("SELECT encrypted_key FROM backup_keys WHERE account_id = ?", (account_id,))
            result = cursor.fetchone()
            return result[0] if result else None
    ...
    def get_used_key(self, platform: str, account_name: str) -> Optional[str]:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("SELECT used_key FROM used_keys WHERE platform = ? AND account_name = ?", (platform, account_name))
            result = cursor.fetchone()
            return result[0] if result else None
    ...
    def archive_used_key(self, platform: str, account_name: str, used_key: str) -> None:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("INSERT INTO used_keys (platform, account_name, used_key) VALUES (?, ?, ?)", (platform, account_name, used_key))
            conn.commit()
    ...
    def get_all_used_keys(self) -> list[tuple[str, str, str]]:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("SELECT platform, account_name, used_key FROM used_keys")
            return cursor.fetchall()
        
    def get_all_accounts(self) -> list[tuple[str, str, str]]:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("SELECT platform, account_name, id FROM accounts")
            return cursor.fetchall()
        
    def get_all_backup_keys(self) -> list[tuple[str, str]]:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("SELECT account_id, encrypted_key FROM backup_keys")
            return cursor.fetchall()
    ...
    def delete_backup_key(self, account_id: str) -> None:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("DELETE FROM backup_keys WHERE account_id = ?", (account_id,))
            conn.commit()
    ...
    def delete_used_key(self, platform: str, account_name: str) -> None:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("DELETE FROM used_keys WHERE platform = ? AND account_name = ?", (platform, account_name))
            conn.commit()
    ...
    def get_key_count(self, account_id: str) -> int:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("SELECT key_count FROM accounts WHERE id = ?", (account_id,))
            result = cursor.fetchone()
    ...
    def increment_key_count(self, account_id: str) -> None:
        with connect(self.db_path) as conn:
            cursor: Cursor = conn.cursor()
            cursor.execute("UPDATE accounts SET key_count = key_count + 1 WHERE id = ?", (account_id,))
            conn.commit()
